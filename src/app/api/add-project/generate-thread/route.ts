      ...
      try {
        ...
        const thread = await openai.beta.threads.create({
          messages: [
            {
              role: "user",
              content: "Please analyze all the uploaded files and give one final output following the instructions of the assistant",
            }
          ],
          tool_resources: {
            file_search: {
        ...
        return NextResponse.json(
          { thread_id: thread.id, projectId: projectId, vectorStoreId: vectorStore.id },
          { status: 200 },
        )
      } catch (err) {
