...
export async function POST(request: NextRequest) {
    try {
        ...
        const run = await openai.beta.threads.runs.createAndPoll(thread.id, {
            ...
            Timestamp: Use the file creation date from metadata or information on asset if available else leave empty. Format as YYYY-MM-DDTHH:MM:SSZ. Avoid using placeholder terms.
        ...
        if (message && message.content[0].type === "text") {
            ...
            try {
                ...
                const newFileData: ProjectFile = {
                    ...
                    timestamp: texts.timestamp ? Timestamp.fromDate(new Date(texts.timestamp)) : Timestamp.now()
